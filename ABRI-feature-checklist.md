...
- [x] **Asks board** — post "Looking for a corporate secretary in KL"; other members answer by naming a business. Post categories in §7. Built Aug 2026 as **Asks** (`/app/asks`), a top-level Workspace item — "Requests" was already taken twice, by connection requests and by the in-flight vouches tab.
  **The pricing row changed, and this is the part to read.** It used to promise "Free and Plus can read it and not reply"; posting and answering are now open to every **SSM-verified (T2)** member on every plan, and what Pro buys is the **alert** — being told an ask matches what you do. Three reasons the paywall moved:
  Gating the *posting* behind Pro would have left the board empty until Pro sold, which is the Introductions failure again. Gating the *answering* would have priced giving, which inverts founding principle #7. And a T2 gate is the one door money can't open (§6), so it doubles as the anti-spam mechanism *and* a reason to get verified.
  Anti-spam is structural, not a moderation queue: T2 to post · **six answers per ask** · one answer per business per ask · self-nomination allowed but *labelled differently*, so pitching yourself can never read as a third-party suggestion. Asks are **not** capped — every comparable platform (Nextdoor, Alignable, LinkedIn, Thumbtack) constrains the answering end, because on a demand board demand is scarce and supply is what arrives as spam.
  An ask closes after 30 days, lazily on read (no cron, same as `vouchExpiry.js`). **Accepting an answer settles the ask and publishes nothing.**
  ~~Accepted answers became a **Recommendation** on the named business's profile, with a T0 growth loop behind it.~~ **Removed Sept 2026**, along with the Recommendations tab, the `recommendation_published` feed event, the recommendation counts and the claim-CTA pull. The reason is one sentence: accepting an answer is the asker settling their own question, and turning that private decision into a public credential on somebody else's profile made one member's choice into another member's evidence. What survives is `AskAnswer.recommendedBusinessId` — the answer still names a business, because that is how the asker knows who to call.
  **What went with it, worth knowing before anyone proposes rebuilding a piece:** the T0 growth loop (a claim CTA reading "3 members have recommended this business") was the only thing pulling unclaimed listings towards claiming, and there is now nothing in its place. If that loop is wanted back, it needs a mechanism that does not publish to a third party — an invite, not a credential.
  `Ask`, `AskAnswer`, `AskFlag` tables · `/asks` and `/admin/ask-reviews` routes · report → freeze → admin queue, copying the `VouchFlag` pattern including derived outcomes.
...
5. ~~**Tags and the requests board.**~~ ▸ *Board done, tags not.* The Asks board ships (§3) and routes on `category` + `location`, which was enough to make it work at seeded density — a routed queue finds people where a scrolling feed would have shown an empty page. Still open in this block: the §7 "Looking for" / "Can offer" tag lists and the Suggested matches screen that reads them.
  Note this block was built **before** step 3's WhatsApp pipe rather than after it, so every ask notification is in-app only (dashboard prompt, sidebar badge, activity feed). That is the one thing the board is missing, and it is a change to the alert, not to the board.
