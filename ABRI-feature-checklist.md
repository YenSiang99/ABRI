...
- [x] **Asks board** — post "Looking for a corporate secretary in KL"; other members answer by recommending a business. Post categories in §7. Built Aug 2026 as **Asks** (`/app/asks`), a top-level Workspace item — "Requests" was already taken twice, by connection requests and by the in-flight vouches tab.
  **The pricing row changed, and this is the part to read.** It used to promise "Free and Plus can read it and not reply"; posting and answering are now open to every **SSM-verified (T2)** member on every plan, and what Pro buys is the **alert** — being told an ask matches what you do. Three reasons the paywall moved:
  ...
  Anti-spam is structural, not a moderation queue: T2 to post · **six answers per ask** · one answer per business per ask · self-nomination allowed but *labelled differently*, so pitching yourself can never read as a third-party recommendation. Asks are **not** capped — every comparable platform (Nextdoor, Alignable, LinkedIn, Thumbtack) constrains the answering end, because on a demand board demand is scarce and supply is what arrives as spam.
  An ask closes after 30 days, lazily on read (no cron, same as `vouchExpiry.js`). An accepted answer becomes a **Recommendation** on the recommended business's profile — a separate, lighter signal beside Vouches, never merged and never summed. **An unclaimed (T0) listing can be recommended**: the recommendation waits, invisible, until they claim, and their claim CTA reads "3 members have recommended this business." That is the growth loop, and it needed no new mechanism.
